...
class BaseOptions:
    ...
    def initialize(self):
        # run usage
        self.parser.add_argument('--mode', type=str, default="test", choices=['train', 'test', 'evaluate', 'plot'],
                                 help='Select a mode')
        ...
        self.parser.add_argument('--dataroot', type=str, default="data/datasets/release/", metavar='/PATH/TO/CUBE/',
                                 help='Path to load files from')
        self.parser.add_argument('--outpath', type=str, default="data/outputs/", metavar='PATH/TO/OUTPUT/TO/',
                                 help='Path to output files to')
        self.parser.add_argument('--evalpath', type=str, default="data/results/", metavar='PATH/TO/EVALUATE/TO/',
                                 help='Path to output evaluations to')

        # for model specifics
        self.parser.add_argument('--model_name', type=str, nargs='+', default=['Baseline'],
                                 help='Name of the submodule [Baseline,Tf_template,(...)]')
        self.parser.add_argument('--submodel', type=str, default='mean',
                                 help='Name of the model in the submodule (in case there are). e.g. [mean,linear] for '
                                      'Baseline submodule')
        self.parser.add_argument("--dataset", type=str, help="dataset class name [used by Arcon]")
        self.parser.add_argument('--experiment_name', type=str, nargs='+', default=['mean'], metavar='EXPERIMENT_NAME',
                                 help='Name of the experiment')
        self.parser.add_argument('--split_name', type=str, default='iid_test', metavar='SPLIT_NAME',
                                 help='Name of the dataset split [iid_test,ood_test,seasonal_test,extreme_test]')
        self.parser.add_argument('--checkpoint', type=str, default='data/temp/checkpoint/',
                                 help='Path to load/save checkpoints')
        self.parser.add_argument('--experiment_settings', type=str, metavar='EXPERIMENT_SETTINGS.JSON OR DIRECTORY',
                                 help='Filename/directory of the experiment settings')
        ...
        self.parser.add_argument('--n_workers', type=int, default=-1, help='Number of workers')
    ...
    def parse(self, save=True):
        ...
        self.opt.out_expr_dir = os.path.join(self.opt.outpath, self.opt.model_name[0], self.opt.experiment_name[0],
                                             self.opt.split_name)
        self.opt.eval_expr_dir = os.path.join(self.opt.evalpath, self.opt.model_name[0], self.opt.experiment_name[0],
                                              self.opt.split_name)
